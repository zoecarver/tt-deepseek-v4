...
def make_lk_d1_kernel(mesh, gamma_cpu, cos_full_cpu, sin_full_cpu):
    ...
    # Swap SUMMA: M=TILE=32, K=N=ROPE_HEAD_DIM=64. Mt=1, Kt=Nt=2.
    # block=(1,1,2), part=(1,2,1) -> 2 cores, M_BPN=N_BPN=1, Kb=1.
    swap_kernel = _make_summa_matmul_kernel(
        M=TILE, K=ROPE_HEAD_DIM, N=ROPE_HEAD_DIM,
        block_cfg=(1, 1, 2), part_cfg=(1, 2, 1))
    rotary_combine_kernel = _make_rotary_combine_kernel(
        num_row_tiles=TILE // TILE,             # 1
        num_h_tiles=ROPE_HEAD_DIM // TILE)      # 2
    ...
    def lk_d1_kernel(kv, cos_full, sin_full, start_pos, kv_out):
        if "scratch" not in state:
            ...
            state["rope_swap_tt"] = _alloc_replicated_zeros((TILE, ROPE_HEAD_DIM))
        ...
        swap_kernel(kv_rope_padded, P_tt, state["rope_swap_tt"])
        rotary_combine_kernel(
            kv_rope_padded, state["rope_swap_tt"], cos_b, sin_b,
            state["rope_rot_tt"])
