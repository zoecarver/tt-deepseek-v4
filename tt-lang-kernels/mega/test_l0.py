...
L0 emits BOTH `a_next` (the post-pad residual stream the next layer's
hc_pre will consume) and `wq_a_out` (the wq_a partial pre-all_gather).

All tt-lang kernel definitions are inlined in this file. Embed prep
(reshape + typecast + repeat + pad) stays as ttnn glue per README
guidance — tt-lang can't typecast.
"""
...
def make_l0_kernel(mesh, hc_fn_cpu, hc_scale_cpu, hc_base_cpu, gamma_cpu):
    """Mega kernel for L0 = embed_prep + hc_pre + attn_norm + wq_a matmul.

    Embed prep (reshape + typecast bf16->fp32 + repeat across mhc + pad)
    stays as ttnn glue per README guidance ("tt-lang cannot handle
    typecasts"). Everything after that mirrors Lk-A's body using locally
    inlined tt-lang kernels.

    Emits:
      a_next:    [num_tokens_pad, D] fp32 — residual stream for next
                 layer's hc_pre.
    ...
    split_mixes_kernel = _make_mhc_split_mixes_kernel(
        num_tiles=NUM_TOKENS_PAD // _MHC_TILE)
    ...
    def l0_kernel(embed_tt, wq_a_w_tt, a_next_out, wq_a_out):
        if "scratch" not in state:
            ...
            state["rms_in_tt"] = _zeros_bf16((TILE, DIM))
        ...
        # 1. Embed prep (ttnn glue): bf16 -> fp32 -> repeat -> pad.
        ...
        # Emit a_next early — this is the residual stream for next layer.
        ttnn.copy(a_tt, a_next_out)
        ...
        # 4. ttnn glue: pull comb's mhc*mhc tail.
        ...
        # 6. mix tile from pre's first mhc cols.
        ...
        # 7. reshape a_tt rows -> [num_tokens*TILE, hidden].
...
def main():
    ...
    try:
        ...
        kernel = make_l0_kernel(mesh, hc_fn, hc_scale, hc_base, gamma)
        ...
        kernel(embed_tt, wq_a_w_tt, a_next_tt, wq_a_out_tt)
